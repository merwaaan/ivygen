...
void Ivy::grow()
{
    ...
    float local_ivySize = mesh->boundingSphereRadius * ivySize;
    float local_maxFloatLength = mesh->boundingSphereRadius * maxFloatLength;
...
Vector3d Ivy::computeAdhesion(const Vector3d &pos)
{
    ...
    float local_maxAdhesionDistance = mesh->boundingSphereRadius * maxAdhesionDistance;

    float minDistance = local_maxAdhesionDistance;


    //find nearest triangle
    for (std::vector<BasicTriangle>::iterator t = mesh->triangles.begin(); t != mesh->triangles.end(); ++t)
    {
...
//
//        //parameters that depend on the scene object bounding sphere
//    float local_ivyLeafSize = mesh->boundingSphereRadius * ivySize * ivyLeafSize;
//
//    float local_ivyBranchSize = mesh->boundingSphereRadius * ivySize * ivyBranchSize;
//
//
